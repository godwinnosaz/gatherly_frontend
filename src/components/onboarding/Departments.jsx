...
const Departments = ({ onNext, onBack }) => {
    ...
    const handleNext = async () => {
        setLoading(true);
        try {
            const selectedDepartments = depts
                .map((dept) => ({
            ...
            const results = await Promise.allSettled(
                selectedDepartments.map((dept) => DepartmentService.create(dept))
            );

            const failed = results.filter((result) => result.status === 'rejected');
            if (failed.length > 0) {
                console.warn('[Onboarding] Some departments were not created:', failed);
            }
        } catch (error) {
            // Endpoint may not be in contract — still advance
