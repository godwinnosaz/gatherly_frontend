...
import { 
  ...
  BrainCircuit, Sparkles, Zap, Command
} from 'lucide-react';
...
const Layout = ({ children }) => {
  ...
  const menuItems = [
    ...
    { icon: BrainCircuit, label: 'Insights', path: '/dashboard/intelligence', roles: ['super_admin', 'pastor', 'fellowship_admin'] },
    { icon: Users, label: 'Members', path: '/dashboard/members', roles: ['super_admin', 'pastor', 'fellowship_admin'] },
    { icon: Calendar, label: 'Events', path: '/dashboard/events', roles: ['all'] },
    { icon: Wallet, label: 'Finances', path: '/dashboard/finance', roles: ['super_admin', 'pastor'] },
        ...
        <header className="h-20 md:h-24 flex items-center justify-between px-6 md:px-12 relative z-40 bg-white/80 backdrop-blur-md border-b border-slate-100">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <Link to="/dashboard/settings" className="relative group">
                ...
                <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-secondary rounded-full border-4 border-white" />
            </Link>
          </div>
        </header>

        {/* Page Content */}
        <div className="flex-1 overflow-y-auto p-6 md:p-12 pb-32 md:pb-12 custom-scrollbar relative z-30">
          ...
          <>
            ...
            >
               <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                 <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-900/20 shrink-0">
                        <Command size={24} />
                    </div>
               ...
               <div className="p-8 border-t border-slate-100 bg-slate-50/50">
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-primary font-black uppercase tracking-widest">{user?.role?.replace('_', ' ')}</p>
                    </div>
                  </div>
                  <button 
                    onClick={handleLogout}
