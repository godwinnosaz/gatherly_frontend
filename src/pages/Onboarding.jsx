...
const Onboarding = () => {
    ...
    const { user, completeOnboarding, markOnboardingComplete, isOnboardingComplete } = useAuth();

    useEffect(() => {
        const fetchStatus = async () => {
            if (!user) return; // Wait for user loading
            
            const scopedKey = getOnboardingKey(user);

            // Check old key just in case it wasn't migrated during checkAuth
            if (localStorage.getItem('gatherly_onboarding_complete') === 'true' && scopedKey) {
                 localStorage.setItem(scopedKey, 'true');
                 localStorage.removeItem('gatherly_onboarding_complete');
            }

            if (isOnboardingComplete && isOnboardingComplete()) {
                completeOnboarding();
        ...
        fetchStatus();
    }, [user, completeOnboarding, isOnboardingComplete, navigate]);
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            // Call the onboarding service to ensure backend record sync
            try {
                await OnboardingService.complete();
            } catch (backendErr) {
                console.error('Backend OnboardingService complete failed:', backendErr);
            }
        } catch (error) {
