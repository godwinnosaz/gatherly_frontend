...
const friendlyMessage = (err) => {
    if (!err) return 'Failed to login. Please try again.';
    // Structured error object from Axios interceptor
    if (err.type === 'unauthenticated') return 'Invalid email or password.';
    if (err.type === 'validation')      return err.message || 'Please check your email and password.';
    if (err.type === 'cors_or_network') return 'Cannot reach the server. Please check your internet connection.';
    if (err.type === 'server_error')    return 'A server error occurred. Please try again in a moment.';
...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // err is the structured object thrown by the Axios interceptor or login()
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center ml-1">
                                ...
                                <Link to="#" className="text-[10px] font-black text-primary hover:underline transition-colors uppercase tracking-widest">
                                    Forgot?
                                </Link>
                            </div>
                            <div className="relative group">
